...
class Criterion(torch.nn.Module):
    ...
    def forward(self, batch, labels, **kwargs):
        ...
        if 'bninception' in self.pars.arch:
            # NPair does not allow for a logsumexp - notation, hence the
            # scaling to avoid overflow!
            batch = batch / 4
        for anchor, positive, negative_set in zip(anchors, positives,
                                                  negatives):
            ...
            loss = loss + torch.mean(
                torch.log(torch.sum(torch.exp(inner_sum), dim=1) +
                          1)) / len(anchors)
            loss = loss + self.l2_weight * torch.mean(
                torch.norm(batch, p=2, dim=1)) / len(anchors)
