...
class Metric():
    def __init__(self, **kwargs):
        self.requires = ['kmeans', 'kmeans_nearest', 'embeds', 'target_labels']
        self.name = 'f1'

    def __call__(self, target_labels, computed_cluster_labels, embeds,
                 centroids):
        import time
        start = time.time()
        if isinstance(embeds, torch.Tensor):
            embeds = embeds.detach().cpu().numpy()
        d = np.zeros(len(embeds))
        for i in range(len(embeds)):
            d[i] = np.linalg.norm(embeds[i, :] -
                                  centroids[computed_cluster_labels[i], :])

        start = time.time()
        labels_pred = np.zeros(len(embeds))
        ...
        for i in range(n_labels):
            count_cluster[i] = len(
                np.where(target_labels == avail_labels[i])[0])
